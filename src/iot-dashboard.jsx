...
const GROUP_MEMBERS = [
  ...
  },
  {
    ...
    curso: "Ciência da Computação",
    semestre: "3º Semestre",
    descricao: "Entusiasta de redes e infraestrutura. Responsável pela montagem física do sensor, configuração do servidor e desenvolvimento do front-end.",
    initials: "KS",
    accent: C.teal,
    accentBg: C.tealDim,
  },
...
const MOCK_ALL_SENSORS = Array.from({ length: 24 }, (_, i) => ({
  id: i + 1,
  local: LOCAIS[i],
  temperatura: +(17 + Math.random() * 15).toFixed(1),
  umidade:     +(33 + Math.random() * 48).toFixed(1),
...
const DataService = {
  ...
  async getAllSensors() {
    ...
    // return res.json();
    return new Promise(r => setTimeout(() => r(MOCK_ALL_SENSORS), 700));
  },
};
